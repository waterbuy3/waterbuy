...
import { categories as FALLBACK_CATS, products as FALLBACK_PRODUCTS, testimonials as FALLBACK_TESTIMONIALS, appStats as FALLBACK_STATS, trustBadges as FALLBACK_BADGES, type Category, type Product } from "@/lib/data";
import { subscribeProducts, subscribeCategories, subscribeHomeContent } from "@/lib/firebase";
import {
  Plus, ChevronRight, Zap, Star, Droplets, CalendarDays,
  Repeat, Package, Flame, Trophy, Users, Clock, ArrowRight,
...
function HomePage() {
  ...
  const [banners, setBanners] = useState<Banner[]>([
    { title: "Get 20% Off on Tanker Booking",  sub: "Use code TANK20",   bg: "from-primary to-water",        badge: "Limited Offer",  emoji: "🚚" },
    { title: "Free Dispenser with 5L Can Sub", sub: "Use code FREEDISP", bg: "from-orange-500 to-amber-400", badge: "New User",       emoji: "🎁" },
    { title: "Flat ₹50 Off on 12-Pack Bundles",sub: "Use code PARTY50",  bg: "from-success to-teal-500",     badge: "Weekend Special",emoji: "🎉" },
  ]);
  ...
  const instantProducts  = products.filter((p) => p.deliveryType === "All" || p.deliveryType === "Instant").slice(0, 6);
    ...
    <div className="bg-muted/30 min-h-screen pb-8">
      ...
      <div className="bg-primary px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold text-white/90">Live · Delivering in your area</span>
        </div>
        <div className="flex items-center gap-3 text-[11px] font-semibold text-white/75">
          <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{appStats.avgDeliveryMin} min avg</span>
          <span className="flex items-center gap-1"><Star className="h-3 w-3 fill-amber-300 text-amber-300" />{appStats.rating}</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {quickActions.map((a) => (
            <Link key={a.label} to={a.to} className="flex flex-col items-center gap-1.5 group">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${a.color} group-active:scale-95 transition-transform shadow-sm`}>
          ...
          <div className="flex">
            {banners.map((banner, idx) => (
              <div key={idx} className="flex-[0_0_100%] min-w-0">
                <div className={`relative bg-gradient-to-r ${banner.bg} h-44 flex items-end p-5 rounded-2xl overflow-hidden wave-bg`}>
                  <div className="z-10 flex-1">
                    ...
                    <h2 className="text-[22px] font-extrabold text-white leading-tight tracking-tight">{banner.title}</h2>
                    <p className="text-xs text-white/80 mt-1 font-semibold">{banner.sub}</p>
                  </div>
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 text-6xl opacity-30 select-none">{banner.emoji}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="px-4 pb-5">
        <div className="flex overflow-x-auto gap-2 no-scrollbar">
          {trustBadges.map((b) => (
            <div key={b.label} className="shrink-0 flex items-center gap-1.5 bg-background border border-border/40 rounded-full px-3 py-1.5 shadow-sm">
              <span className="text-sm">{b.icon}</span>
              <span className="text-[11px] font-bold text-foreground whitespace-nowrap">{b.label}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="grid grid-cols-3 divide-x divide-white/10">
          {[
            ...
            <div key={i} className="flex flex-col items-center py-4 gap-0.5">
              ...
              <span className="text-[9px] text-white/50 font-semibold uppercase tracking-wide text-center leading-tight px-2">{s.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-4 mb-5 bg-background rounded-2xl shadow-sm border border-border/40 overflow-hidden">
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          <h2 className="font-extrabold text-foreground text-sm tracking-tight">SHOP BY CATEGORY</h2>
          <Link to="/products" className="text-[11px] font-bold text-primary flex items-center gap-0.5">
            All <ChevronRight className="h-3 w-3" />
          </Link>
        </div>
        <div className="grid grid-cols-4 gap-y-5 gap-x-2 px-4 pb-5">
          {categories.map((cat) => (
            <Link key={cat.id} to="/products" search={{ category: cat.id }} className="flex flex-col items-center gap-1.5 group">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl bg-gradient-to-b ${cat.color} shadow-sm group-active:scale-95 transition-transform`}>
                {cat.icon}
              </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  <span className="text-[11px] font-extrabold text-white/80 uppercase tracking-wider">3-Day Streak!</span>
                </div>
            ...
            <div className="flex items-center justify-between">
              <p className="text-[11px] text-white/70 font-semibold">
                {hydration >= hydrationGoal ? "🎉 Goal reached!" : `${(hydrationGoal - hydration).toFixed(1)}L more to go`}
              </p>
              <button onClick={() => setHydration((h) => Math.min(+(h + 0.25).toFixed(2), 3))}
      ...
      <section className="mb-5">
        ...
        <div className="flex overflow-x-auto gap-3 pb-2 no-scrollbar px-4">
          {instantProducts.map((product) => {
            ...
            return (
              <div key={product.id} className="w-[148px] shrink-0 bg-background rounded-2xl border border-border/40 shadow-sm relative overflow-hidden card-lift group">
                {product.badge && (
                ...
                <div className="h-28 bg-muted rounded-t-2xl overflow-hidden">
                  <img src={product.imageUrl} alt={product.name} width={160} height={112} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-3">
                  <h3 className="font-bold text-foreground text-xs line-clamp-2 leading-tight mb-1">{product.name}</h3>
                  <p className="text-[10px] text-muted-foreground font-medium mb-1">{product.size}</p>
                  {product.rating && <div className="mb-2"><StarRating rating={product.rating} /></div>}
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-extrabold text-sm text-foreground">₹{product.price}</span>
                      {product.mrp && <span className="text-[10px] text-muted-foreground line-through ml-1">₹{product.mrp}</span>}
                    </div>
                    {qty === 0 ? (
                      ...
                      <div className="flex items-center gap-1 bg-primary rounded-lg px-1.5 py-0.5">
                        <button onClick={() => removeFromCart(product.id)} className="text-white text-sm font-bold px-0.5">−</button>
                        <span className="text-white text-xs font-extrabold w-4 text-center">{qty}</span>
                        <button onClick={() => addToCart(product.id)} className="text-white text-sm font-bold px-0.5">+</button>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section className="mx-4 mb-5">
        ...
        <div className="space-y-3">
          {featuredProducts.map((p) => {
            ...
            return (
              <div key={p.id} className="flex gap-3 bg-background rounded-2xl border border-border/40 p-3 shadow-sm card-lift">
                <div className="w-20 h-20 rounded-xl overflow-hidden bg-muted shrink-0">
                  <img src={p.imageUrl} alt={p.name} width={80} height={80} loading="lazy" className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 flex flex-col justify-between py-0.5">
                  <div>
                    <span className="text-[9px] font-extrabold text-primary uppercase tracking-widest">{p.badge}</span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {qty === 0 ? (
                      ...
                      <div className="flex items-center gap-1 bg-primary rounded-lg px-2 py-1">
                        <button onClick={() => removeFromCart(p.id)} className="text-white text-sm font-bold">−</button>
                        <span className="text-white text-xs font-extrabold w-4 text-center">{qty}</span>
                        <button onClick={() => addToCart(p.id)} className="text-white text-sm font-bold">+</button>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section className="mb-5">
        <div className="flex items-center justify-between px-4 mb-3">
          ...
          <div className="flex items-center gap-1 bg-amber-50 border border-amber-200 px-2 py-1 rounded-full">
            <Star className="h-3 w-3 fill-amber-400 text-amber-400" />
            <span className="text-[11px] font-extrabold text-amber-700">{appStats.rating} · {appStats.happyCustomers}</span>
          </div>
        </div>
        <div className="flex overflow-x-auto gap-3 pb-2 no-scrollbar px-4">
          {testimonials.map((t) => (
            <div key={t.id} className="shrink-0 w-64 bg-background border border-border/40 rounded-2xl p-4 shadow-sm card-lift">
              ...
              <p className="text-xs text-foreground/80 font-medium leading-relaxed line-clamp-3">"{t.text}"</p>
              <span className="mt-2 inline-block text-[9px] font-extrabold text-primary bg-primary/10 px-2 py-0.5 rounded-full">{t.tag}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <Link to="/subscriptions">
          <div className="bg-foreground rounded-2xl p-5 flex items-center justify-between shadow-water-lg relative overflow-hidden group">
            ...
            <div className="relative z-10 max-w-[72%]">
              ...
              <h2 className="text-lg font-extrabold text-background mb-1 tracking-tight leading-tight">Never run out of water again</h2>
              <p className="text-[11px] text-background/70 font-medium">Subscribe from ₹899/month · Cancel anytime</p>
            </div>
          ...
          <Link to="/checkout">
            <div className="bg-success text-white rounded-2xl px-4 py-3 flex items-center shadow-2xl justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-extrabold">{totalItems} item{totalItems !== 1 ? "s" : ""} in cart</p>
                  <p className="text-xs font-semibold opacity-80">₹{totalPrice} total</p>
                </div>
              </div>
              <span className="font-extrabold text-sm bg-white/20 rounded-xl px-3 py-1.5">Checkout →</span>
            </div>
          </Link>
        </div>
      )}
